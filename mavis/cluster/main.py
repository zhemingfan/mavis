    ...
    **kwargs
    ...
    breakpoint_pairs = read_inputs(
        ...
        cast={
            ...
            if not config[SECTION]['split_only ']
            else x
        },
        add_default={
            ...
            COLUMNS.tools: '',
            COLUMNS.disease_status: config['libraries'][library]['disease_status'],
    ...
    for bpp in breakpoint_pairs:
        ...
        if bpp.library != library:
        ...
        elif not config[SECTION]['limit_to_chr'] or (
            bpp.break1.chr in config[SECTION]['limit_to_chr']
            and bpp.break2.chr in config[SECTION]['limit_to_chr']
        ):
            ...
            filtered_pairs.append(bpp)
    if config[SECTION]['limit_to_chr']:
        other_chr -= set(config[SECTION]['limit_to_chr'])
    ...
    if config[SECTION]['uninformative_filter']:
        ...
        pass_clusters, uninformative_clusters = filter_uninformative(
            annotations.content, breakpoint_pairs, max_proximity=config[SECTION]['max_proximity']
        )
        LOG(
            'filtered from',
    ...
    if not config[SECTION]['split_only ']:
        ...
        clusters = merge_breakpoint_pairs(
            breakpoint_pairs,
            cluster_radius=config[SECTION]['cluster_radius'],
            cluster_initial_size_limit=config[SECTION]['cluster_initial_size_limit'],
        )
    ...
    output_files = split_clusters(
        breakpoint_pairs,
        output,
        min_clusters_per_file=config[SECTION]['min_clusters_per_file'],
        max_files=config[SECTION]['max_files'],
